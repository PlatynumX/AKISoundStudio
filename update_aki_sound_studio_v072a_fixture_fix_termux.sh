...
ZIP_NAME="AKISoundStudio-v0.7.2a-source-fixture-fix.zip"
REPO_NAME="AKISoundStudio"
WORK="$HOME/aki-sound-studio-v070-update"
ARTIFACT="AKISoundStudio-v0.7.2a-win64"
...
LOG_FILE="$DOWNLOADS/AKISoundStudio-v0.7.2a-termux-update.log"
: > "$LOG_FILE"
  ...
  for root in "${roots[@]}"; do
    ...
    while IFS= read -r -d '' candidate; do
      ...
      find "$root" -maxdepth 5 -type f \
        \( -iname 'AKISoundStudio-v0.7.2a-source*.zip' \
           -o -iname '*AKISoundStudio*v0.7.2a*source*.zip' \) \
        -print0 2>/dev/null
    )
  done
...
ZIP_PATH="$(resolve_source_zip)" || fail "Could not find the AKISoundStudio v0.7.2a source ZIP anywhere in shared phone storage."
...
# main in place and avoids creating a v0.7.2a or any other separate branch.
...
if git diff --cached --quiet; then
  info "The main branch already contains this v0.7.2a source"
else
  git commit -m "AKI Sound Studio v0.7.2a"
fi
...
if ! gh run watch "$RUN_ID" --repo "$REPO" --exit-status; then
  BUILD_LOG="$DOWNLOADS/AKISoundStudio-v0.7.2a-failed-build-$RUN_ID.log"
